...
public class FhirTaskDaoImpl extends BaseDaoImpl implements FhirTaskDao {
	...
	public Collection<FhirTask> searchForTasks(ReferenceParam basedOnReference, ReferenceParam ownerReference,
		...
		if (!(ownerReference == null || ownerReference.getIdPart() == null || ownerReference.getResourceType() == null)) {
			Criteria ownerCriteria = criteria.createCriteria("ownerReference");
			ownerCriteria.add(Restrictions.eq("reference", ownerReference.getIdPart()));
			ownerCriteria.add(Restrictions.eq("type", ownerReference.getResourceType()));
		}
		// Task.status
